...
namespace WindowsRecentFilesFilterer {
   ...
   public class AppContext : System.Windows.Forms.ApplicationContext, IDisposable  {
      ...
      public void ShowError(string message, Exception ex, [CallerMemberName] string caller = null, [CallerLineNumber] int line = -1, [CallerFilePath] string file = null) {
			...
			statements.Add("Message: " + (message.IsNotWhitespace() ? message : NotAvailable));
			statements.Add("Reason: " + ((ex?.Message).IsNotWhitespace() ? ex?.Message : NotAvailable));
			if((ex?.InnerException?.Message).IsNotWhitespace()) { statements.Add("Inner Reason: " + ex.InnerException.Message); }
			...
			statements.Add("\t Called By Method: " + (caller.IsNotWhitespace() ? caller : NotAvailable));
			statements.Add("\t Called From File: " + (file.IsNotWhitespace() ? file : NotAvailable));
			if(ex != null) {
				statements.Add(SectionBreak);
